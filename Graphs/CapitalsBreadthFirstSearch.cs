...
public class CapitalsBreadthFirstSearch
{
    private Vertex[] _vertices;
    
    public Vertex[] Execute(CapitalsUndirectedGraph g, int s)
    {
        ...
        while (q.Count > 0)
        {
            ...
            var sortedAdj = g.Adj(u.V).OrderBy(v => g.GetKm(u.V, v)).ToList();

            foreach (var v in sortedAdj)
            {
                int newDistance = u.Distance + g.GetKm(u.V, v); 
                
                if (_vertices[v].Color == Color.WHITE)
                {
                    _vertices[v].Color = Color.GRAY;
                    _vertices[v].Distance = u.Distance + 1; //newDistance;
